...
describe('Core Logic Tests', () => {
  ...
  describe('Card Validation', () => {
    it('should validate card structure', () => {
      ...
      expect(validCard.front).toBeTruthy()
      expect(validCard.back).toBeTruthy()
      expect(validCard.createdAt).toBeInstanceOf(Date)
      expect(validCard.updatedAt).toBeInstanceOf(Date)
      expect(validCard.nextReview).toBeInstanceOf(Date)
      expect(typeof validCard.interval).toBe('number')
      expect(typeof validCard.ease).toBe('number')
      expect(Array.isArray(validCard.reviewHistory)).toBe(true)
    })

    it('should detect invalid cards', () => {
      const invalidCard = {
        id: 'test-id',
        front: '', // Empty front
        back: 'Test answer',
      ...
      expect(invalidCard.front).toBeFalsy()
    })
  })
